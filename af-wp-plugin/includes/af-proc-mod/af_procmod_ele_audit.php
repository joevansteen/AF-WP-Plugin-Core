	 	...
	 	foreach ($typeList as $relLabel => $relType)
			...
			if ($entries > 0)
				...
				foreach ($dataArray as $key => $value)
					{
					$eleDoc = $this->anchor->getSavedAirDocument($key, NULL);
...
//							$this->resultMsg->attachDiagnosticTextItem($itemName, 'ElementData has no detail');
	...
	function procDbConvertCreate()
		...
		if (!empty($testValue))
			{
			$manifestName 	= $this->anchor->getRefName($testValue).' ';
			}
...
*********/
	...
	function procDbConvertRegister()
		...
		if (! $missingManifest)
			 ...
			 */
			$manifest = $this->anchor->getSavedAirDocument($cvtManifest, NULL);
...
//					$manifestEntry['Converted']	= $collectionItemNode->getChildContentByName('Converted');
		...
		$this->anchor->putTraceData(__LINE__, __FILE__, 'manifest list = '.count($cvtMasterList));
			...
			$cvtTimePt2			= time();
			$rowCount	= $this->anchor->myDbLayer->getCount_AirRelRules();
			if ($rowCount > 0)
				{
				$dbData		= $this->anchor->myDbLayer->get_AllRelRules($rowCount);
				if ((!isset($dbData))
				 || (is_null($dbData))
				 || (!is_array($dbData)))
					{
				...
				if ($ruleCount > 0)
					...
					foreach ($dbData as $dbRow)
						...
						$success		= $this->anchor->myDbLayer->insert_AirRelRulesItem($newArray);
						}
					}
				}
			$cvtTimePt3			= time();
			$rowCount	= $this->anchor->myDbLayer->getCount_AirProperties();
			if ($rowCount > 0)
				{
				$dbData		= $this->anchor->myDbLayer->get_AllProperties($rowCount);
				if ((!isset($dbData))
				 || (is_null($dbData))
				 || (!is_array($dbData)))
					{
				...
				if ($propCount > 0)
					...
					foreach ($dbData as $dbRow)
						...
						$success		= $this->anchor->myDbLayer->insert_AirElePropertiesItem($newArray);
						}
					}
				}
			$cvtTimePt4			= time();
			$rowCount	= $this->anchor->myDbLayer->getCount_AirAssociations();
			if ($rowCount > 0)
				{
				$dbData		= $this->anchor->myDbLayer->get_AllAssociations($rowCount);
				if ((!isset($dbData))
				 || (is_null($dbData))
				 || (!is_array($dbData)))
					{
				...
				if ($assocCount > 0)
					...
					foreach ($dbData as $dbRow)
						...
						$success		= $this->anchor->myDbLayer->insert_AirAssociationsItem($newArray);
						}
					}
				}
			$cvtTimePt5			= time();
			$rowCount	= $this->anchor->myDbLayer->getCount_AirRelationships();
			if ($rowCount > 0)
				{
				$dbData		= $this->anchor->myDbLayer->get_AllRelationships($rowCount);
				if ((!isset($dbData))
				 || (is_null($dbData))
				 || (!is_array($dbData)))
					{
				...
				if ($relationCount > 0)
					...
					foreach ($dbData as $dbRow)
						...
						$success		= $this->anchor->myDbLayer->insert_AirEleRelationshipsItem($newArray);
						}
					}
				}
			}
	...
	function procCleanseDb()
		...
		$cvtTimePt1				= time();
	 	...
	 	/****************************************************************************/
		$cvtTimePt2			= time();
		$rowCount	= $this->anchor->myDbLayer->getCount_AirElements();
		if ($rowCount > 0)
			{
			$dbData		= $this->anchor->myDbLayer->get_AllElements($rowCount);
			if ((!isset($dbData))
			 || (is_null($dbData))
			 || (!is_array($dbData)))
				{
		...
		$cvtTimePt2			= time();
		$rowCount	= $this->anchor->myDbLayer->getCount_AirEleIndex();
		if ($rowCount > 0)
			{
			$dbData		= $this->anchor->myDbLayer->get_AllIndexItems($rowCount);
			if ((!isset($dbData))
			 || (is_null($dbData))
			 || (!is_array($dbData)))
				{
		...
		if ($rowCount > 0)
			...
			if ($ruleCount > 0)
				...
				if ($ruleRowsDeleted > 0)
					{
					$success		= $this->anchor->myDbLayer->purge_AirRelRulesSet($eleDelete);
					}
				}
			}
		$cvtTimePt3			= time();
		...
		if ($rowCount > 0)
			...
			if ($propCount > 0)
				...
				if ($propRowsDeleted > 0)
					{
					$success		= $this->anchor->myDbLayer->purge_AirPropertiesSet($eleDelete);
					}
				}
			}
		$cvtTimePt4			= time();
		...
		if ($rowCount > 0)
			...
			if ($assocCount > 0)
				...
				if ($assocRowsDeleted > 0)
					{
					$success		= $this->anchor->myDbLayer->purge_AirAssociationsSet($eleDelete);
					}
				}
			}
		$cvtTimePt5			= time();
	...
	function procDbGetStats()
		...
		$eleCount			= $this->anchor->myDbLayer->getCount_AirElements();
		$indexCount			= $this->anchor->myDbLayer->getCount_AirEleIndex();
		...
		$propCount			= $this->anchor->myDbLayer->getCount_AirProperties();
		$assocCount			= $this->anchor->myDbLayer->getCount_AirAssociations();
		$relationCount		= $this->anchor->myDbLayer->getCount_AirRelationships();
...
//					$manifestEntry['Converted']	= $collectionItemNode->getChildContentByName('Converted');
		...
		$cvtTimePt2			= time();
		 	...
		 	foreach ($cvtElementList as $cvtElement)
				...
				if ($manifestSize > 0)
		 		 ...
		 		 */
				$oldDbDoc = $this->anchor->getSavedAirDocument($cvtElement, NULL);
		...
		$this->anchor->putTraceData(__LINE__, __FILE__, $replCount.' key alterations identified');
				 ...
				 */
				$newAirDoc = new C_AirElementDoc($this->anchor);
