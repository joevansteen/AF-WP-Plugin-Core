<?php
/*
 * dlg_FileCreate.fmt script Copyright (c) 2005 Architected Futures, LLC
 *
 * V1.3 2005-OCT-16 JVS Begin test of new standalone PHP environment script
 * V1.3 2005-NOV-15 JVS Integration with old Tikiwiki AIR base
 * V1.7 2008-MAR-03 JVS Code conversion to PHPv5.2
 * V5   2019-APR-04 JVS EATSv5 required script include segment
 */

// Insure a correct execution context ...
$myProcClass = 'C_AirFileCreatePanel';
$myDynamClass = $myProcClass;	
require(AF_AUTOMATED_SCRIPTS.'/af_script_header.php');

class C_AirFileCreatePanel extends C_HtmlPanel {
	/***************************************************************************
	 * Constructor
	 *
	...
	function initialize($panelClass = '')
		...
		$air_ContentType			= $this->anchor->getDlgVar('air_ContentType');
...
//		$this->setHiddenElement('dlgPanelType',	$dlgPanelType);
		...
		$this->pageTitle = 'Element Compose';

		$myTitleData = new C_AirHtmlElement($this);
		$myTitleData->initialize(htmlEleH1, null, 'Element Compose');
		...
		$work				= & $myTabData->createChild(htmlEleInput);
									...
									$work->setAttribute(htmlAttrName, 'fileName');
									$work->setAttribute(htmlAttrMaxLength, '240');
									$work->setAttribute(htmlAttrSize, $textareaWidth);
									$work->setAttribute(htmlAttrValue, $fileName);
		$this->anchor->sessionDoc->putDialogVarTracker('fileName', 'Item'); // Catalog the dialog form variables
		...
		$work				= & $myTabData->createChild(htmlEleSelect);
									$work->setAttribute(htmlAttrName, 'fileType');
									foreach ($air_ContentType as $key => $value)
										{
										$item		= & $work->createChild(htmlEleOption, $value['mimeType']);
		...
		$this->anchor->sessionDoc->putDialogVarTracker('fileType', 'Item'); // Catalog the dialog form variables
