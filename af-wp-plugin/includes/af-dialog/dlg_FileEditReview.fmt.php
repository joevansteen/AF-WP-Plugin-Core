<?php
/*
 * dlg_FileEditReview.fmt script Copyright (c) 2005 Architected Futures, LLC
 *
 * V1.4 2005-NOV-24 JVS Bootstrap from dlg_FileView.fmt
 * V1.7 2008-MAR-03 JVS Code conversion to PHPv5.2
 * V5   2019-APR-04 JVS EATSv5 required script include segment
 */

// Insure a correct execution context ...
$myProcClass = 'C_AirFileEditReviewPanel';
$myDynamClass = $myProcClass;	
require(AF_AUTOMATED_SCRIPTS.'/af_script_header.php');

class C_AirFileEditReviewPanel extends C_HtmlPanel {
	/***************************************************************************
	 * Constructor
	 *
...
//		$air_ItemHeader			= $this->anchor->getDlgVar('air_ItemHeader');
		...
		$editContent				= $this->anchor->getDlgVar('FileContent');
		$origContent				= $this->anchor->getDlgVar('OrigContent');
...
//		$this->setHiddenElement('dlgPanelType',	$dlgPanelType);
		...
		$this->pageTitle = 'Element Edit Review';
...
/***************************************
		$myTable		= & $pageBody->createChild(htmlEleTable);
		$myTabRow		= & $myTable->createChild(htmlEleTblRow);
		$myTabData		= & $myTabRow->createChild(htmlEleTblData, $origContent);
								 $myTabData->setAttribute(htmlAttrAlign, 'left');
		***********************************/

		$frame			= & $pageBody->createChild(htmlEleDefList);
		$def				= & $frame->createChild(htmlEleDefTerm, 'New Content');
		$data				= & $frame->createChild(htmlEleDefDescr);
		$work				= & $data->createChild(htmlEleTextarea, $editContent);
...
//		$this->anchor->sessionDoc->putDialogVarTracker('eleNewContent', 'Item'); // Catalog the dialog form variables
		...
		if (!empty($origContent))
			{
			$frame			= & $pageBody->createChild(htmlEleDefList);
			$def				= & $frame->createChild(htmlEleDefTerm, 'Old Content');
			$data				= & $frame->createChild(htmlEleDefDescr);
			$work				= & $data->createChild(htmlEleTextarea, $origContent);
									$work->setAttribute(htmlAttrRows, $textareaHeight);
									$work->setAttribute(htmlAttrCols, $textareaWidth);
									$work->setAttribute(htmlAttrReadonly);
			}
