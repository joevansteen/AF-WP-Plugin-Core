...
require(AF_AUTOMATED_SCRIPTS.'/af_script_header.php');
	 ...
	 ***************************************************************************/
class C_MenuElement extends C_AirObjectBase {
	...
	public function initialize($role, $itemId, $parentElement)
		{
	 	if (($parentElement != NULL)
	 	 && (!($parentElement instanceof C_MenuElement)))
		 	{
	...
	private function addChild(& $childNode)
	 	{
	 	if ($childNode	instanceof C_MenuElement)
		 	{
	 ...
	 * initializeChildScan()
	 *
	 * Resets the cursor for scanning the child elements.
	 *******/
	public function initializeChildScan()
	 	{
	 	reset($this->children);
		}
	 ...
	 * firstChild()
	 *
	 * Retrieves the first child of the element, or false if there are no children.
	 *******/
	 ...
	 * nextChild()
	 *
	 * Retrieves the next child of the element, or false if there are no more children.
	 *******/
	public function nextChild()
	 	{
		$item = each ( $this->children );
      if ($item)
      	{
         return ( $item['value'] );
	      }
