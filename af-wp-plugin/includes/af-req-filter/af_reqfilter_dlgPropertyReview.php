<?php
/*
 * af_reqfilter_dlgPropertyReview script Copyright (c) 2008 Architected Futures, LLC
 *
...
$myProcClass = 'C_ReqFilterPropertyReview';
$myDynamClass = $myProcClass;	
require(AF_AUTOMATED_SCRIPTS.'/af_script_header.php');


class C_ReqFilterPropertyReview extends C_AF_ReqFilterBase {
	...
	function procPropertyReview()
		...
		switch ($txnAction)
			...
			case AIR_Action_Submit:
				...
				$selection = $this->procMsg->getMessageData('dlgAction');
				...
				if (! is_null($dlgEleType))
	   			...
	   			if (($this->procContext->getSessionData('txnContextEleType') != $dlgEleType)
	   			 ...
	   			 && ($dlgEleType != AIR_Null_Identifier))
		   			...
		   			$eleTypeDoc = & $this->anchor->Repository->getElementRef($dlgEleType);
			   	...
			   	case AIR_Action_PurgeType:
			   		 ...
			   		 */
						if (! is_null($dlgEleType))
							...
							$itemCount = $this->anchor->Repository->airDB->getTypeCount_AirEleIndex($dlgEleType);
							if (! $itemCount)
								{
									$name = $this->anchor->Repository->getElementName($dlgEleType);
